...
box::use(
  ...
  leaflet.minicharts[addMinicharts, popupArgs, updateMinicharts],
  htmltools[HTML],
  htmlwidgets[onRender],
  RColorBrewer[brewer.pal],
  grDevices[col2rgb, convertColor],
  dplyr[
    mutate,
    filter,
    summarise,
  ...
  layout_sidebar(
    sidebar = sidebar(
      ...
      selectInput(
        ns("type"),
        "Chart type",
        choices = c("pie", "bar", "polar-area", "polar-radius")
      ),
...
mod_seabass_telemetry_data_server <- function(
  ...
  moduleServer(id, function(input, output, session) {
    ...
    chart_layer_id <- paste0(
      anim_df$station_name,
      "__",
      format(anim_df$month, "%Y%m")
    )
    ...
    make_legend_html <- function(id_vec) {
      if (length(id_vec) == 0) {
        return(
          "<div class='id-legend-scroll'><i>No individuals detected</i></div>"
        )
      }
      items <- paste0(
        "<div style='display:flex;align-items:center;gap:6px;margin:2px 0;'>",
        "<span style='width:12px;height:12px;display:inline-block;background:",
        id_palette[id_vec],
        ";'></span>",
        "<span>",
        id_vec,
      ...
      vapply(
        ...
        function(r) {
          ...
          header <- paste0(
            ...
            station_vec[r],
            "</b><br>",
            format(month_vec[r], "%Y-%m"),
            "<br>"
          )
          if (length(nz) == 0) {
            return(paste0(header, "<i>No detections</i>"))
          }
